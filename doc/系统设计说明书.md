...
```
sys-design
├── context-model -s <file>           # 上下文模型 CRUD 操作
│   ├── add
...
│   └── show <id>
│
├── logic-model -s <file>             # 逻辑模型 CRUD 操作
│   ├── add
│   │   ├── system <id> [-n NAME] [-d DESC]
...
│   └── show <id>
│
├── concept-model -s <file>           # 概念模型（层次规则定义）
│   ├── add
│   │   ├── element <TYPE>            # 添加元素类型
...
```bash
# 1. 添加系统定义
sys-design context-model -s model.yaml add system SNP_SYS -n "SNP System" -d "核心系统"

# 2. 添加外部系统
sys-design context-model -s model.yaml add external-system BD_SOFT -n "BD Soft" --tech "REST API"

# 3. 添加角色
sys-design context-model -s model.yaml add actor USER -n "用户" -t external

# 4. 添加接口
sys-design context-model -s model.yaml add interface ITF_API -n "API Interface" -p rest

# 5. 定义接口关系
sys-design context-model -s model.yaml add provide-relation SNP_SYS ITF_API
sys-design context-model -s model.yaml add interface-usage USER ITF_API
...
sys-design concept-model -s model.yaml add level SUBSYSTEM -n "subsystem" -c COMPONENT
sys-design concept-model -s model.yaml add level COMPONENT -n "component" -c MODULE
sys-design concept-model -s model.yaml add level MODULE -n "module" -c SUBMODULE
sys-design concept-model -s model.yaml add level SUBMODULE -n "submodule" -c SUBMODULE

# 2. 添加元素类型
sys-design concept-model -s model.yaml add element subsystem
sys-design concept-model -s model.yaml add element component
sys-design concept-model -s model.yaml add element module
sys-design concept-model -s model.yaml add element submodule

# 3. 添加逻辑视图元素
sys-design logic-model -s model.yaml add system MY_SYSTEM -n "My System"
sys-design logic-model -s model.yaml add subsystem CTRL_SUBSYSTEM -n "Controller Subsystem"
sys-design logic-model -s model.yaml add component CTRL -n "Controller" --subsystem CTRL_SUBSYSTEM
sys-design logic-model -s model.yaml add module MOTOR_CTRL -n "Motor Controller"

# 4. 定义包含关系
sys-design logic-model -s model.yaml add containment MY_SYSTEM CTRL_SUBSYSTEM
sys-design logic-model -s model.yaml add containment CTRL_SUBSYSTEM CTRL
sys-design logic-model -s model.yaml add containment CTRL MOTOR_CTRL

# 5. 添加接口和依赖
sys-design logic-model -s model.yaml add interface ITF_MOTOR -n "Motor Interface"
sys-design logic-model -s model.yaml add provide-relation MOTOR_CTRL ITF_MOTOR
sys-design logic-model -s model.yaml add dependency MOTOR_CTRL ITF_SPEED

# 6. 验证逻辑视图
sys-design validate -s model.yaml -t logic-view
...
# 列出所有子系统
sys-design logic-model -s model.yaml list subsystems

# 列出概念模型层次
sys-design concept-model -s model.yaml list

# 查看元素详情
sys-design context-model -s model.yaml show SNP_SYS
sys-design logic-model -s model.yaml show CTRL_SUBSYSTEM
```
